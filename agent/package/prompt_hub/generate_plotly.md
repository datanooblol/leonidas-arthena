...
- **CRITICAL**: When using texttemplate, use %{y} for y-values, not %{text} unless explicitly setting text parameter
- For JSON serialization compatibility, avoid complex texttemplate expressions that reference undefined fields
- **DEPRECATED**: Do not use titlefont, use title.font instead. Avoid deprecated Plotly properties
...
def create_chart(data):
    ...
    # Example: For categorical comparisons
        ...
        # For bar charts with value labels - use %{y} for JSON compatibility
